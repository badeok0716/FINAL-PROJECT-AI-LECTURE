...
def get_tokenlized(file, num_ratio = 1.0):
    """tokenlize the file"""
    tokenlized = list()
    with open(file) as raw:
        raw_ = list(raw)
        num_data = int(num_ratio * len(raw_))
        raw_ = raw_[:num_data]
        for text in raw_:
...
def get_tokenlized(file, num_ratio=1.0):
    ...
    with open(file) as raw:
        l_raw = len(list(raw))
        num_data = int(num_ratio * l_raw)
        print(l_raw,num_data)
        raw_ = list(raw)[:num_data]
        for text in raw:
            print(text)
            text = nltk.word_tokenize(text.lower())
            tokenlized.append(text)
    print(file,len(tokenlized))
    return tokenlized
'''
