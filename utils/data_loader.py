...
class GenDataIter:
    def __init__(self, samples, num_ratio=1.0, if_test_data=False, shuffle=None):
        ...
        self.loader = DataLoader(
            dataset=GANDataset(self.__read_data__(samples, num_ratio)),
    ...
    def __read_data__(self, samples,num_ratio):
        """
        ...
        if isinstance(samples, torch.Tensor):  # Tensor
            ...
            inp, target = self.load_data(samples, num_ratio)
            all_data = [{'input': i, 'target': t} for (i, t) in zip(inp, target)]
        else:
            all_data = None
        return all_data
    ...
    def load_data(self, filename, num_ratio=1.0):
        """Load real data from local file"""
        self.tokens = get_tokenlized(filename, num_ratio=num_ratio)
        print(filename, len(self.tokens))
