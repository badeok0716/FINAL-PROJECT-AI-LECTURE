...
def init_param(opt):
    ...
    aug_place = opt.aug_place
    
    if opt.aug_place == 'all':
        aug_gen_fake = True
        aug_gen_real = True
        aug_dis_fake = True
        aug_dis_real = True
    elif opt.aug_place == 'D_only':
        aug_gen_fake = False
        aug_gen_real = False
        aug_dis_fake = True
        aug_dis_real = True  
    elif opt.aug_place == 'D_only2':
        aug_gen_fake = False
        aug_gen_real = True
        aug_dis_fake = True
        aug_dis_real = True  
    elif opt.aug_place == 'real_only':
        aug_gen_fake = False
        aug_gen_real = True
        aug_dis_fake = False
        aug_dis_real = True  
    ...
    eval_b_num = int( opt.eval_b_num / opt.train_ratio )
    ...
    MLE_train_epoch = int( opt.mle_epoch / opt.train_ratio )
    ...
    d_step = int( opt.d_step / opt.train_ratio )
    d_epoch = opt.d_epoch
    if run_model == 'relgan':
        ADV_d_step = opt.adv_d_step
    else:
        ADV_d_step = int( opt.adv_d_step / opt.train_ratio )
    ...
    # Save path
    if opt.key == 'aug_exp' or opt.key == 'aug_exp_test' or opt.key == 'aug_exp_final':
        save_root = 'save/{}/{}/{}/{}_{}/{}_temp{}_trial{}/'.format(opt.key,dataset,run_model,opt.aug,opt.aug_place,opt.train_ratio,temperature,opt.trial)
    else:
        save_root = 'save/{}/{}/{}_{}_dt-{}_lt-{}_mt-{}_et-{}_sl{}_temp{}_lfd{}_T{}/'.format(time.strftime("%Y%m%d"),
                                                                                            dataset, run_model, model_type,
                                                                                            d_type,
